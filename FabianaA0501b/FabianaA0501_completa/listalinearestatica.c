...
/**
* Testa se uma lista é um palíndromo.
* @param lst - Lista a ser testada.
*/

void testarPalind(struct Lista lst){
    struct Lista lisTeste; // lista "recipiente" que reberá lst invertida
    int i = 0, k = 0, cont = 0;

    iniciarLista(&lisTeste); // inicializa listTeste na memória
    for(i = 0; i < lst.n; i++)
        inserir(&lisTeste, i, lst.elem[i]);

    // invertendo lisTeste
    inverter(&lisTeste);
    for(k = 0; k < i; k++){
        if(lisTeste.elem[k] == lst.elem[k]){
            cont++;
        }
    }
...
/**
* Percorre uma lista removendo os elementos repetidos.
* @param lst - Lista a ser testada.
*/

void tirarIguais(struct Lista* lst){
    int i = 0;
    int j = 0;
